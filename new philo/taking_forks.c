...
/*   Updated: 2021/11/19 21:36:56 by nlutsevi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		philo_takes_forks(t_philo *philo)
{
	...
	if (((last_time == 0) && ((philo[philo->num].taken_left_fork - data->start_time) > data->time_to_die)) || \
		((last_time != 0) && (philo[philo->num].taken_left_fork - philo[philo->num].start_eat) > data->time_to_die))
	{
		philo_died(philo);
		return (1);
	}
...
int		philo_takes_right_fork(t_philo *philo)
{
	...
	if ((philo->num + 1) != data->num_philos)
	{
	...
	if (((last_time == 0) && ((philo[philo->num].taken_right_fork - data->start_time) > data->time_to_die)) || \
		((last_time != 0) && (philo[philo->num].taken_right_fork - philo[philo->num].start_eat) > data->time_to_die))
	{
		philo_died(philo);
		return (1);
	}
...
void	print_right_fork(t_philo *philo)
{
	...
	time = philo[philo->num].taken_right_fork - data->start_time;
...
void	print_left_fork(t_philo *philo)
{
	...
	time = philo[philo->num].taken_left_fork - data->start_time;
	pthread_mutex_lock(&philo->data->mutex_print);
	if (philo->data->muerte != 1)
